...
export interface ArInvoiceItemInput {
  ItemID: number;
  ItemCode?: string;
  ItemName?: string;
  Quantity: number;
...
export class ArInvoiceService {
  ...
  public async getArInvoiceById(id: number) {
  ...
  public async createArInvoice(data: ArInvoiceInput, createdById: number) {
  ...
  /** Update AR Invoice */
  public async updateArInvoice(id: number, data: ArInvoiceInput, updatedById: number) {
    const existing = await prisma.arInvoice.findUnique({
      where: { ID: id },
      include: { items: true },
    });
    if (!existing) throw new NotFoundError('AR Invoice not found.');
  ...
  /** Delete AR Invoice */
  public async deleteArInvoice(id: number) {
