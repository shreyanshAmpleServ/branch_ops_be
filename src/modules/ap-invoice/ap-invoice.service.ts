...
export interface ApInvoiceItemInput {
  ItemID: number;
  ItemCode?: string;
  ItemName?: string;
  Quantity: number;
...
export interface ApInvoiceAttachmentInput {
  Attachment: string;
}
...
export interface ApInvoiceInput {
  ...
  series?: string;
  BOL?: string;
  items: ApInvoiceItemInput[];
  attachments?: ApInvoiceAttachmentInput[];
}

export class ApInvoiceService {
  /** Get all AP invoices with filters */
  public async getApInvoices(params: {
    search?: string;
    status?: string;
    startDate?: string;
    endDate?: string;
    ...
    if (params.startDate || params.endDate) {
      where.PostDate = {};
      if (params.startDate) where.PostDate.gte = new Date(params.startDate);
      if (params.endDate) where.PostDate.lte = new Date(params.endDate);
    }
      ...
      new Set(
        invoices
          .map(inv => {
            ...
            return [inv.CreatedBy, numCust].filter((id): id is number => id !== null && id !== undefined);
          })
          .flat()
      )
    );
    ...
    const custCodes = invoices.map(i => i.CustCode).filter(Boolean) as string[];
    ...
    if (allCguids.length > 0) {
      ...
      for (const itm of allItems) {
        if (itm.mainCguid) {
          if (!itemsMap[itm.mainCguid]) itemsMap[itm.mainCguid] = [];
  ...
  public async getApInvoiceById(id: number) {
  ...
  public async createApInvoice(data: ApInvoiceInput, createdById: number) {
  ...
  /** Update AP Invoice */
  public async updateApInvoice(id: number, data: ApInvoiceInput, updatedById: number) {
  ...
  /** Delete AP Invoice */
  public async deleteApInvoice(id: number) {
    const existing = await prisma.apInvoice.findUnique({
      where: { ID: id },
    });
