...
export class BranchesService {
  public async getBranches(search?: string, activeOnly?: boolean): Promise<Branch[]> {
    ...
    try {
      const branches: any[] = await prisma.$queryRawUnsafe("SELECT * FROM dbo.Branches ORDER BY code ASC");
  ...
  public async getBranchById(id: number): Promise<Branch | null> {
    try {
      const result: any[] = await prisma.$queryRawUnsafe("SELECT * FROM dbo.Branches WHERE id = @P1", id);
  ...
  public async createBranch(code: string, name: string, address: string = '', active: boolean = true): Promise<Branch> {
    const activeVal = active ? 1 : 0;
    await prisma.$executeRawUnsafe(
      "INSERT INTO dbo.Branches (code, name, address, active) VALUES (@P1, @P2, @P3, @P4)",
      code, name, address, activeVal
    );
    const result: any[] = await prisma.$queryRawUnsafe("SELECT TOP 1 * FROM dbo.Branches ORDER BY id DESC");
    const b = result[0];
    return {
      id: b.id,
      code: b.code ?? '',
  ...
  public async updateBranch(id: number, data: { code?: string, name?: string, address?: string, active?: boolean }): Promise<Branch | null> {
    ...
    await prisma.$executeRawUnsafe(
      "UPDATE dbo.Branches SET code = @P1, name = @P2, address = @P3, active = @P4 WHERE id = @P5",
      code, name, address, activeVal, id
    );

    return this.getBranchById(id);
  }

  public async deleteBranch(id: number): Promise<boolean> {
    try {
