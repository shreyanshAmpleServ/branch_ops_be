...
export class QuotationsService {
  ...
  private async generateQuotCode(): Promise<string> {
  ...
  public async createQuotation(data: QuotationInput, createdById: number) {
  ...
  /** Update quotation */
  public async updateQuotation(id: number, data: QuotationInput, updatedById: number) {
    const existing = await prisma.quotations.findUnique({
      where: { ID: id },
      include: { items: true },
    });
    if (!existing) throw new NotFoundError('Sales Quotation not found.');
