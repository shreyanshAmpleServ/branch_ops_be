...
export class ItemPricesService {
  ...
  public async createPriceList(data: { code?: string; name: string; priceType?: number }) {
  ...
  public async getItemPrices(params: {
    page?: number;
    limit?: number;
    search?: string;
    priceListId?: number;
    ...
    const rows: ItemPriceDto[] = itemPricesList.map((ip: any) => {
      const item = ip.Items || {};
  ...
  /** Upsert Item Price */
  public async upsertItemPrice(data: {
    priceListId: number;
    itemId: number;
    price: number;
    currency?: string;
  ...
  public async bulkUpdatePrices(data: {
    priceListId: number;
    updates: { itemId: number; price: number }[];
  }) {
    const results = [];
