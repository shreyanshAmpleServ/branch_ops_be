...
export interface IncomingPaymentInvoiceInput {
...
export interface IncomingPaymentMeansInput {
  means_type: string; // 'cash' | 'bank' | 'check' | 'credit'
  gl_account: string;
  amount: number;
  transfer_date?: string;
  reference_num?: string;
  check_bank_code?: string;
  cc_name?: string;
  is_cleared?: boolean;
}
...
export interface IncomingPaymentInput {
  ...
  journal_remarks?: string;
  total_amount: number;
  currency?: string;
  status?: string;
  invoices?: IncomingPaymentInvoiceInput[];
  means?: IncomingPaymentMeansInput[];
}

export interface OutgoingPaymentInvoiceInput {
...
export interface OutgoingPaymentMeansInput {
  means_type: string; // 'bank' | 'cash' | 'check'
  gl_account: string;
  amount: number;
  transfer_date?: string;
  reference_num?: string;
...
export interface OutgoingPaymentInput {
  ...
  project_code?: string;
  cost_center?: string;
  invoices?: OutgoingPaymentInvoiceInput[];
  means?: OutgoingPaymentMeansInput[];
}

export interface PettyCashLineInput {
...
export interface PettyCashClaimInput {
  claim_number?: string;
...
export class BankingService {
   ...
   * ========================================================================= */
  public async getBankingOverview() {
   ...
   * 2. INCOMING PAYMENTS
   * ========================================================================= */
  public async getIncomingPayments(params: {
    search?: string;
    status?: string;
    startDate?: string;
    endDate?: string;
    ...
    if (params.startDate || params.endDate) {
      where.posting_date = {};
      if (params.startDate) where.posting_date.gte = new Date(params.startDate);
      if (params.endDate) where.posting_date.lte = new Date(params.endDate);
    }
  ...
  public async getIncomingPaymentById(id: number) {
    ...
    if (!payment) throw new NotFoundError('Incoming Payment not found');
  ...
  private async generateIncomingDocNumber(): Promise<string> {
    const year = new Date().getFullYear().toString().slice(-2);
    const count = await prisma.incoming_payments.count();
    return `IP${year}/${String(count + 1).padStart(4, '0')}`;
  }

  public async createIncomingPayment(data: IncomingPaymentInput, createdById: number) {
  ...
  public async deleteIncomingPayment(id: number) {
    const existing = await prisma.incoming_payments.findUnique({ where: { id } });
    if (!existing) throw new NotFoundError('Incoming Payment not found');
  ...
  public async getAvailableInvoices(customerCode?: string) {
    const where: any = {
   ...
   * 3. OUTGOING PAYMENTS
   * ========================================================================= */
  public async getOutgoingPayments(params: {
    search?: string;
    status?: string;
    startDate?: string;
    endDate?: string;
  ...
  public async getOutgoingPaymentById(id: number) {
    ...
    if (!payment) throw new NotFoundError('Outgoing Payment not found');
  ...
  private async generateOutgoingDocNumber(): Promise<string> {
    const year = new Date().getFullYear().toString().slice(-2);
    const count = await prisma.outgoing_payments.count();
    return `OP${year}/${String(count + 1).padStart(4, '0')}`;
  }

  public async createOutgoingPayment(data: OutgoingPaymentInput, createdById: number) {
  ...
  public async deleteOutgoingPayment(id: number) {
    const existing = await prisma.outgoing_payments.findUnique({ where: { id } });
    if (!existing) throw new NotFoundError('Outgoing Payment not found');
   ...
   * 4. PETTY CASH
   * ========================================================================= */
  public async getPettyCashAccounts() {
    const accounts = await prisma.petty_cash_accounts.findMany({
      orderBy: { id: 'asc' },
      include: {
  ...
  public async getPettyCashClaims(params: {
    search?: string;
    status?: string;
    category?: string;
    startDate?: string;
    ...
    if (params.startDate || params.endDate) {
      where.request_date = {};
      if (params.startDate) where.request_date.gte = new Date(params.startDate);
      if (params.endDate) where.request_date.lte = new Date(params.endDate);
    }
  ...
  public async getPettyCashClaimById(id: number) {
    ...
    if (!claim) throw new NotFoundError('Petty Cash Claim not found');
  ...
  public async createPettyCashClaim(data: PettyCashClaimInput, userId: number) {
  ...
  public async disbursePettyCashClaim(id: number, accountId: number, approvedById: number) {
    ...
    if (!account) throw new NotFoundError('Petty Cash Account not found');
   ...
   * ========================================================================= */
  public async getPendingPoPayments(params: { search?: string; status?: string }) {
    const where: any = {};
    if (params.status && params.status !== 'all') {
      where.status = params.status;
    }
