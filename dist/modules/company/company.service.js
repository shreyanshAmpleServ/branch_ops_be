...
export class CompanyService {
    async getCompanyDetails() {
        try {
            const rows = await prisma.$queryRawUnsafe("SELECT * FROM dbo.CompanyDetails");
            if (rows.length === 0)
                return null;
            const b = rows[0];
            return {
    ...
    async updateCompanyDetails(payload) {
        ...
        WHERE CompanyName = @P20`, companyAddr, printHeader, phone1, phone2, fax, email, localCur, systemCur, tin, vrn, smtpEmail, smtpPassword, smtpServer, smtpPort, smtpType, headerColor, leftMenuColor, compUrl, compLogo, companyName);
