...
function mapItem(item, catMap, whsMap, itemWhsMap, fallbackWhsId) {
...
export class ItemsService {
    ...
    async getItems(params) {
        ...
        const itemWhsMap = new Map(itemWhsEntries.map((iw) => [iw.ItemID, iw.WhsID]));
        const fallbackWhsId = warehouses[0]?.ID || 1;
        const mappedItems = items.map((item) => mapItem(item, catMap, whsMap, itemWhsMap, fallbackWhsId));
        // Stats calculations
    ...
    async getItemById(id) {
        ...
        const catMap = new Map(categories.map((c) => [c.ID, c.Name]));
        const whsMap = new Map(warehouses.map((w) => [w.ID, w.Name]));
        const itemWhsMap = itemWhs ? new Map([[itemWhs.ItemID, itemWhs.WhsID]]) : undefined;
        const fallbackWhsId = warehouses[0]?.ID || 1;
    ...
    async updateItem(id, data) {
        ...
        if (data.catId !== undefined)
            updateData.CatID = data.catId;
        if (data.subCatId !== undefined)
            updateData.SubCatId = data.subCatId;
        if (data.dfltWhsId !== undefined)
            updateData.DfltWhsID = data.dfltWhsId;
        if (data.uom !== undefined)
            updateData.UoM = data.uom;
        if (data.qtyInCase !== undefined)
            updateData.QtyinCase = data.qtyInCase;
        if (data.minQtyLevel !== undefined)
            updateData.MinQtyLevel = data.minQtyLevel;
        if (data.maxQtyLevel !== undefined)
            updateData.MaxQtyLevel = data.maxQtyLevel;
        if (data.lastPurPrc !== undefined)
            updateData.LastPurPrc = data.lastPurPrc;
        if (data.weight !== undefined)
            updateData.Weight = data.weight;
        if (data.saleVAT !== undefined)
            updateData.SaleVAT = data.saleVAT;
        if (data.remarks !== undefined)
            updateData.Remarks = data.remarks;
        if (data.posItem !== undefined)
            updateData.POSItem = data.posItem;
        if (data.itemPurchased !== undefined)
            updateData.ItemPurchased = data.itemPurchased;
        if (data.itemSales !== undefined)
            updateData.ItemSales = data.itemSales;
        if (data.itemInventory !== undefined)
            updateData.ItemInventory = data.itemInventory;
        if (data.onHand !== undefined)
            updateData.OnHand = data.onHand;
